...
<section class="pt-32 pb-8 border-b border-white/[0.08]">
    <div class="max-w-5xl mx-auto px-5">
        <div class="font-mono text-xs text-amber-500 mb-4">$ mail --compose</div>
        <h1 class="text-3xl sm:text-4xl font-bold text-white tracking-tight">Kontak</h1>
        <p class="mt-3 text-white/60 max-w-lg">Punya masukan untuk project saya atau ada hal lain yang ingin dibahas? Kirim pesan saja. Biasanya saya membalas dalam 1–2 hari.</p>
    </div>
</section>
...
<section class="py-10 pb-24">
    <div class="max-w-5xl mx-auto px-5">
        <div class="grid grid-cols-1 lg:grid-cols-[.8fr_1.2fr] gap-8">
            ...
            <div class="space-y-5">
                <div class="card p-6">
                    <h2 class="font-mono text-xs text-amber-500 mb-5">// info kontak</h2>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="font-mono text-xs">
                            <div class="text-white/35 mb-1">{{ $contact['label'] }}</div>
                            @if($contact['href'])
                                <a href="{{ $contact['href'] }}" class="text-sm text-white hover:text-amber-400 transition-colors">{{ $contact['value'] }}</a>
                            @else
                ...
                <div class="card p-6">
                    <h2 class="font-mono text-xs text-amber-500 mb-4">// social</h2>
                    <div class="space-y-2">
                        @if($profile->github_url)
                        <a href="{{ $profile->github_url }}" target="_blank" class="flex items-center gap-3 p-3 border border-white/[0.08] hover:border-white/20 hover:bg-white/[0.03] transition-colors text-white">
                        ...
                        <a href="{{ $profile->linkedin_url }}" target="_blank" class="flex items-center gap-3 p-3 border border-white/[0.08] hover:border-white/20 hover:bg-white/[0.03] transition-colors text-white">
            ...
            <div>
                <div class="card p-6 sm:p-8">
                    <h2 class="font-mono text-xs text-amber-500 mb-1">// kirim pesan</h2>
                    ...
                    <form action="{{ route('contact.store') }}" method="POST" id="contact-form" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="name" class="block font-mono text-xs text-white/50 mb-1.5">nama</label>
                                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                                       class="input-field @error('name') !border-red-500/50 @enderror">
                                @error('name') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="email" class="block font-mono text-xs text-white/50 mb-1.5">email</label>
                        ...
                        <div>
                            <label for="subject" class="block font-mono text-xs text-white/50 mb-1.5">subjek</label>
                        ...
                        <div>
                            <label for="message" class="block font-mono text-xs text-white/50 mb-1.5">pesan</label>
                ...
                <div class="card p-6 mt-5">
                    <h3 class="font-mono text-xs text-amber-500 mb-4">// faq</h3>
                    <div class="space-y-2">
                        @foreach($faqs as $faq)
                        <details class="group border border-white/[0.08]">
