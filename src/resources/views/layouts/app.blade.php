                ...
                <div class="flex justify-between items-center h-16">
                    ...
                    <a href="{{ route('portfolio.home') }}" class="flex items-center gap-2 group">
                        <div class="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-bold text-sm group-hover:bg-indigo-500 transition-colors">
                            {{ $globalProfile ? collect(explode(' ', $globalProfile->name))->map(fn($n) => $n[0])->first() : 'P' }}
                        </div>
                        <span class="text-white font-bold text-lg tracking-tight">{{ $globalProfile->name ?? 'Zefhana Ananda' }}</span>
                    </a>
        ...
        <footer class="bg-slate-900 text-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                    <div class="md:col-span-2">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h3 class="text-xl font-bold">{{ $globalProfile->name ?? 'Zefhana Ananda' }}</h3>
                        </div>
                        <p class="text-slate-400">Portofolio profesional {{ $globalProfile->name ?? 'Zefhana Ananda' }} — {{ $globalProfile->title ?? 'Full-Stack Developer' }}.</p>
                        <div class="flex gap-4 mt-6">
                            @if($globalProfile->github_url ?? null)
                            <a href="{{ $globalProfile->github_url }}" target="_blank" class="w-10 h-10 rounded-lg bg-slate-800 hover:bg-indigo-600 flex items-center justify-center transition-colors" aria-label="GitHub">
                            ...
                            @endif
                            @if($globalProfile->linkedin_url ?? null)
                            <a href="{{ $globalProfile->linkedin_url }}" target="_blank" class="w-10 h-10 rounded-lg bg-slate-800 hover:bg-indigo-600 flex items-center justify-center transition-colors" aria-label="LinkedIn">
                ...
                <div class="border-t border-slate-800 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <p class="text-slate-500 text-sm">&copy; {{ date('Y') }} {{ $globalProfile->name ?? 'Zefhana Ananda' }}. All rights reserved.</p>
